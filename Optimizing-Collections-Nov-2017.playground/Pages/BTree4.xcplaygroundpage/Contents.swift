/*:
 ...
 Whenever we had to implement COW, we did it by implementing `makeFooUnique`
 methods that create clones of shared storage when necessary. Our code then
 proceeded to mutate the resulting unique storage in a separate, second step.
 
 If you think about it, this is a rather inefficient way of doing this: for
 example, when we have to insert a new element at the start of a shared
 `OrderedSet` value, `makeUnique` first makes a brand-new `NSOrderedSet` with
 the exact same elements, and then `insert` proceeds to immediately move all of them
 one slot to the right to make room for the new item. 
 
 No wonder `sharedInsert` was so slow for `OrderedSet`s! It would be much faster
 to create the clone of the shared storage such that it already has the new
 element inserted at the correct place. We won't implement this for
 `OrderedSet` though, as it isn't worth the effort: it would still remain far
 behind our other `SortedSet` implementations.
 
 However, we can observe the same suboptimal behavior in all three `BTree`
 variants we've made thus far. When we insert an element into a shared B-tree node,
 we start by copying all existing elements into a new buffer; we insert the new
 value in a separate step. What's even worse is the insertion may result in an
 oversized node, which then needs to be split — and this is done as yet another
 separate operation, moving half of the elements into a third buffer. A single
 insertion may therefore copy/move several hundred elements not once, not twice,
 but *three different times*! That seems inefficient.
 
 It's possible to eliminate all this needless copying/moving by merging
 `makeUnique`, `insert`, and `split` into a single, rather complicated operation.
 Merging `makeUnique` and `insert` can be done by implementing a non-mutating
 variant of insertion and switching to it when we detect the node is
 shared. To unify this hybrid insertion with `split`, we need to detect that a
 split will be necessary *before* doing the insertion, and if so, create two
 nodes from scratch so that the new element is already in the correct place.
 Note that the new element may end up inside the first node, or inside the
 second node, or — if it happens to fall exactly in the middle — it may become
 the separator value between the two halves of the original node. Thus, to unify
 all three operations, we need to write $2 \times 4 = 8$ separate variants of
 insertion.
 
 I did my best to implement this; unfortunately, the result was a wash. Shared
 insertion became a little bit faster for certain element counts, and it became
 a little bit slower for others. In-place insertion didn't change much at all.
 
 I won't include the code for my attempt for `BTree4` here, but [you'll find it
 in the GitHub repository][BTree4]. Try tweaking it; maybe you'll succeed at
 making it meaningfully faster!
